...
const SalesProcess = () => {
  ...
  // const isRefresh = store.get('isRefresh');
  const [activeTab, setActiveTab] = useState('')
  const [showDetails, setShowDetails] = useState(false)

  store.set('processDoc', 1)
  ...
  const fetchSubMenuList = () => {
    const returnData = SubMenuService(referenceId, processDoc, tenantId)
    return returnData
  }

  useEffect(() => {
    async function onLoadFunc() {
      const response = await fetchSubMenuList(referenceId, processDoc, tenantId)
    ...
    onLoadFunc()
  }, [referenceId, processDoc, tenantId])
  ...
  useEffect(() => {
    ...
    if (staticTab != null) {
      ...
      store.set('slaveId', staticTab.slaveId)
  ...
  // useEffect(() => {
  //   const currentTab = subMenu && subMenu.find(data => data.component === activeTab.component)
  //   console.log(currentTab,"---hi")
  // }, [isRefresh, activeTab])
