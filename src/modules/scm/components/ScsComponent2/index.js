...
const SupCompState = ({ componentData, visibling, isView, onmodalCancel, ProcessCode1 }) => {
  console.log(componentData, 'component data in suscompstate')
  const hdrId = componentData?.igHdrId
  const indentType = componentData?.sbcDesc
  const subAssy = componentData?.pskDesc
  const station = componentData?.pkDesc
  const indentId = componentData?.indentId || componentData?.indentID
  const scsStatus = componentData?.scsStatus
  const totalcost = componentData?.targetCost
  const indentcode = componentData?.indentCode
  // Multi-indent (station) groups: indentType/subAssy/indentcode arrive as a single
  // GROUP_CONCAT'd string. Show a count + breakdown popover instead, same as the PJS list.
  const indentCount = Number(componentData?.indentCount) || 0
  const multiIndent = indentCount > 1
  ...
  const getscsretrievaldata = async () => {
    ...
    if (httpgetdetails.responseCode === '200') {
      ...
      if (scpVendorListdata && scpVendorListdata.length > 0) {
        ...
        console.log('Vendor check for blocked', allPropForm.getFieldValue('vendorshortl1'))
  ...
  const handlescsapproval = async seq => {
    ...
    try {
      const insertCheck = await safeHandleInsert()
      let isValid = false
  ...
  const handlevendorchange = (value, option, name) => {
    ...
    if (name === 'vendorshortl1') {
      ...
      setCountryL1(option.country.trim().toLowerCase() === 'india')
      ...
      setCountryL2(option.country.trim().toLowerCase() === 'india')
      ...
      setCountryL3(option.country.trim().toLowerCase() === 'india')
    }
    setLoading(true)
  ...
  // Same-part rows pulled from multiple indents in this PJS are shown here as one row so pricing
  // only has to be entered once. `priceTable` (the real per-indent rows used for submission,
  // grand totals, etc.) is never restructured - this only builds a display/allocation view over
  // it, same pattern as the station-scoped merge in AddIndentGroup.
  ...
  const buildMergedPriceRows = rows => {
    ...
    byProduct.forEach(group => {
      ...
      // so the merged row's own Extended Price is simply rate x total qty - no need to read back
      // each source's individually-stored extended value.
  ...
  const handleRateChangeFx = (record, fxRateField, rsRateField, fxExtField, rsExtField, value, isIndia, level) => {
    ...
    const exchangeRateField = { L1: 'exchangeRate1', L2: 'exchangeRate2', L3: 'exchangeRate3' }[level]
  ...
  const MemoizedTable = useMemo(() => {
    return (
      <Form
        ...
        initialValues={{ priceTable: mergedPriceRows }}
      >
        <Table
  ...
  // "ⓘ" popover next to Indent No. on the header, for a PJS raised on a multi-indent
  // (station) group - reuses the same endpoint the PJS list's breakdown popover uses.
  ...
  // indentType/subAssy arrive as a single ", "-joined distinct list (GROUP_CONCAT) for
  // multi-indent groups - split it so the ⓘ popover can show each value on its own line.
    ...
    <div>
      <div>
        <Modal
          ...
          width="900"
          onCancel={() => {
            // X button / Esc route through here — ignore them while Save/Approve/Previous
            // Stage/Delete PJS is in flight so the form can't be dismissed mid-request.
                            ...
                            {multiIndent ? (
                              <span>
                                ...
                                <Popover
                                  ...
                                  content={renderListPopoverContent(indentType)}
                                >
                                  <InfoCircleOutlined style={{ color: '#1890ff', cursor: 'pointer' }} />
                                </Popover>
                              </span>
                            ...
                            {multiIndent ? (
                              <span>
                                ...
                                <Popover
                                  ...
                                  content={renderListPopoverContent(subAssy)}
                                >
                                  <InfoCircleOutlined style={{ color: '#1890ff', cursor: 'pointer' }} />
                                </Popover>
                              </span>
                        ...
                        <div className="col-12 col-sm-12 col-md-3 col-lg-3 col-xl-3 col-xxl-3">
                          <div style={{ display: 'flex', alignItems: 'center' }}>
                            ...
                            <p style={{ marginBottom: '0' }}>
                              {scmHdrdata &&
                              scmHdrdata.length > 0 &&
                              scmHdrdata[0].totalBudgetConsumed
                                ? parseFloat(
                        ...
                        <div className="col-12 col-sm-12 col-md-3 col-lg-3 col-xl-3 col-xxl-3">
                          <div style={{ display: 'flex', alignItems: 'center' }}>
                            ...
                            <p style={{ marginBottom: '0' }}>
                              {scmHdrdata &&
                              scmHdrdata.length > 0 &&
                              scmHdrdata[0].actualConsumedValue
                                ? parseFloat(
                        ...
                        <tr>
                          ...
                          >
                            <CustomFormInput
                              name="exchangeRate1"
                              disable={
                                allPropForm.getFieldValue('country1')?.toLowerCase() === 'india' ||
                                formdisable
                              }
                              // disabled={country1?.trim()?.toLowerCase() === 'india' || formdisable}
                              maxLength={64}
                            />
                          </td>
                          <td
                          ...
                          >
                            <CustomFormInput
                              name="exchangeRate2"
                              disable={
                                allPropForm.getFieldValue('country2')?.toLowerCase() === 'india' ||
                                formdisable
                              }
                              // disabled={country2?.trim()?.toLowerCase() === 'india' || formdisable}
                              maxLength={64}
                            />
                          </td>
                          <td
                          ...
                          >
                            <CustomFormInput
                              name="exchangeRate3"
                              disable={
                                allPropForm.getFieldValue('country3')?.toLowerCase() === 'india' ||
                                formdisable
                              }
                              // disabled={country3?.trim()?.toLowerCase() === 'india' || formdisable}
                              maxLength={64}
                            />
                          </td>
                        </tr>
                          ...
                          <tr>
                            ...
                            >
                              <CustomFormNumberInput
                                ...
                                readOnly={
                                  isEditable === '0' || isEditable === undefined || countryL1
                                }
                                maxLength={15}
                              />
                            </td>
                            ...
                            >
                              <CustomFormNumberInput
                                ...
                                readOnly={
                                  isEditable === '0' || isEditable === undefined || countryL2
                                }
                                maxLength={15}
                              />
                            </td>
                            ...
                            >
                              <CustomFormNumberInput
                                ...
                                readOnly={
                                  isEditable === '0' || isEditable === undefined || countryL3
                                }
                                maxLength={15}
                              />
                            </td>
