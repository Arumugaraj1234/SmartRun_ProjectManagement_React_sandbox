...
const InventoryStockModal = ({ selectedMonth, project, check }) => {
  ...
  const getProjectDataDtls = async () => {
    ...
    const httpresponse = await InspectionReportService({
      requestPath: 'getIndentByInventoryStock',
      requestData: props,
    })
    if (httpresponse.responseCode === '200') {
      const updatedList = httpresponse.responseData.map((item, index) => ({
        ...item,
        sno: index+1,
        type: item.type ?? 'Inventory Stock'
      }));
...
const indentCode1 = []
  ...
  const type2 = type1.filter(distinctval)
  ...
  const projectColumns = [
    ...
    },
    {
      ...
      key: 'type',
      filters: type3,
      filteredValue: filterData.type,
      onFilter: (value, record) => record?.type === value,
      render: (text) => text || 'Inventory Stock', 
    },
    {
      title: 'Qty',
      dataIndex: 'qty',
    ...
    },
    {
      ...
      filteredValue: filterData.deliveryDate,
      onFilter: (value, record) => record?.deliveryDate === value,
