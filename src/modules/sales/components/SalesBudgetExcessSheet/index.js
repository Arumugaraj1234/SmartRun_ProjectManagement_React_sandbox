...
const SalesBudgetExcessSheet = () => {
  ...
  // numeric arrays → sort numerically
  actualCost2
    .slice()
    .sort((a, b) => a - b)
    .forEach(element =>
  ...
  // tableData (not searchedData) so a filter/search that happens to isolate one sibling still finds
  // its true siblings and includes every one of them in the merged action.
...
export default SalesBudgetExcessSheet
