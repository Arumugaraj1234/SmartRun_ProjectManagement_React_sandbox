...
const ItemsDelayedModal = ({ selectedMonth, project, check }) => {
  ...
  projectData.map(h => {
    return plannedStartDate1.push(h.createdDateTime)
  })
  projectData.map(h => {
  ...
  projectData.map(h => {
    return plannedCompletedDate1.push(h.expectedDeliveryDate)
  })
  projectData.map(h => {
  ...
  projectData.map(h => {
    return deliveryDate1.push(h.deliveryDate)
  })
  projectData.map(h => {
  ...
  const plannedStartDate2 = plannedStartDate1.filter(distinctval)
  const plannedCompletedDate2 = plannedCompletedDate1.filter(distinctval)
  ...
  const employeeName2 = employeeName1.filter(distinctval)
  ...
  plannedCompletedDate2.map(element => {
    return plannedCompletedDate3.push({
      text: element,
      value: element,
    })
  })
  ...
  const projectColumns = [
    ...
    },
    {
      ...
      filteredValue: filterData.employeeName,
    ...
    //   filteredValue: filterData.createdDateTime,
    //   onFilter: (value, record) => record?.createdDateTime === value,
    ...
    //   filters: plannedCompletedDate3,
    //   filteredValue: filterData.expectedDeliveryDate,
    //   onFilter: (value, record) => record?.expectedDeliveryDate === value,
    //   render: text => (text ? moment(text).format('DD-MMM-YYYY') : '-'),
    // },
    {
      title: 'No Of Delay Days',
      dataIndex: 'delay',
      key: 'delay',
      className: 'right-align-cell',
      render: (text, record) => {
        const diff = moment(record.expectedDeliveryDate).diff(moment(), 'days')
  ...
  const handleExportCSV = () => {
    ...
    downloadCSV(csvData, `Items_Delayed_${currentDateTime}.csv`)
  }
  ...
  const cleanupDataSource = dataSource => {
    return dataSource.map(row => {
      ...
      const diff = row.expectedDeliveryDate
        ? moment(row.expectedDeliveryDate).diff(moment(), 'days')
        : ''
      ...
      return {
        ...
        'Indent Line Item Assigned Person': escapeValue(row.employeeName),
        'No Of Delay Days': escapeValue(diff),
      }
    })
  }
...
export default ItemsDelayedModal
